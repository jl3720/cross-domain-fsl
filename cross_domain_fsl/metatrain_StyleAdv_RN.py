...
from cross_domain_fsl.data.datamgr import SimpleDataManager, SetDataManager
from cross_domain_fsl.methods.StyleAdv_RN_GNN import StyleAdvGNN

from cross_domain_fsl.options import parse_args, get_resume_file, load_warmup_state
from cross_domain_fsl.test_function_fwt_benchmark import test_bestmodel
from cross_domain_fsl.test_function_bscdfsl_benchmark import test_bestmodel_bscdfsl
...
def train(base_loader, val_loader, model, start_epoch, stop_epoch, params):
    ...
    for epoch in range(start_epoch, stop_epoch):
        model.train()
        total_it = model.train_loop(
            epoch, base_loader, optimizer, total_it
...
if __name__ == "__main__":
    ...
    model = StyleAdvGNN(
        model_dict[params.model], tf_path=params.tf_dir, **train_few_shot_params
    )
    model = model.cuda()
    ...
    if params.resume != "":
        ...
        if resume_file is not None:
            ...
            print(
                "  resume the training with at {} epoch (model file {})".format(
