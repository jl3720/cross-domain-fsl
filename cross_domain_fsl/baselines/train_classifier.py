...
class StyleContentDataset(torch.utils.data.Dataset):
    def __init__(self, K, N, LEN_prompt, DIM):
        ...
        self.input_data_1 = torch.randn(size=(K * N, LEN_prompt, DIM), dtype=torch.float16)
        self.input_data_1 = [self.input_data_1[i] for i in range(self.input_data_1.shape[0])]
        ...
        self.input_data_2 = [self.input_data_2[i] for i in range(self.input_data_2.shape[0])]
...
class ArcMarginProduct(nn.Module):
    ...
    def __init__(self, in_features, out_features, s=30.0, m=0.50, easy_margin=False):
        ...
        self.weight = Parameter(torch.FloatTensor(out_features, in_features).to(torch.float16))
...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts):
        ...
        # x.shape = [batch size, n ctx，transformer.width]
...
def train_classifier(input_data_1, input_data_2, clip_model_name, dataset_name, args):
    ...
    save_path =  classifier_dir / '{}_{}_classifier.pth'.format(clip_model_name.replace('/', '_'), dataset_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--clip-vision-model', type=str, default='RN50', help='Valid options (case sensitive): RN50, RN101, RN50x4, ViT-B/32, ViT-B/16, ViT-L/14')
    parser.add_argument('--dataset', type=str, default='PACS')
    parser.add_argument('--style-vecs-dir', type=str, default='./pth/style_vecs_out', help='Directory containing style vectors')
    parser.add_argument('--classifier-dir', type=str, default='./pth/classifiers', help='Directory to save classifier')

    args = parser.parse_args()
    print("##################################################")
    print(f"Training classifier with {args.clip_vision_model} CLIP backbone on {args.dataset} dataset")
    print("##################################################")
    print(f"args: {args}")
    ...
    file_name = style_vecs_dir / '{}_{}_style_vecs_out'.format(clip_model_name.replace('/', '_'), dataset_name)
