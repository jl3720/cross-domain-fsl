# Copied from `finetune_StyleAdv_RN.py`. But want to load CLIP backbone,
# add a linear layer, and create the ProtoNet object.
# Use the `model.set_forward` function to get the features.
...
class AdaptCLIP(nn.Module):
    def __init__(self):
        super(AdaptCLIP, self).__init__()
        backbone, transform = clip.load("ViT-B/32", DEVICE)
        ...
        self.clip = backbone
        self.linear = nn.Linear(512, 128)  # CLIP_DIM_MAPPING: 512

        self.transform = transform  # TODO: should be the cdfsl transform?

    def forward(self, x):
        x = self.clip.encode_image(self.transform(x))
        x = self.linear(x)
        return x
...
def finetune(novel_loader, n_pseudo=75, n_way=5, n_support=5):
    ...
    checkpoint_dir = "%s/checkpoints/%s/best_model.tar" % (
        params.save_dir,
        params.resume_dir,
    )
    # state = torch.load(checkpoint_dir)["state"], don't load meta trained model
    BACKBONE_NAME = "ViT-B/32"
    backbone = clip.load(BACKBONE_NAME, DEVICE)

    for ti, (x, _) in enumerate(novel_loader):  # x:(5, 20, 3, 224, 224)
        model = StyleAdvGNN(
            model_dict[params.model], n_way=n_way, n_support=n_support
        ).cuda()
        ...
        torch.cuda.empty_cache()
        print(
            "Task %d : %4.2f%%, mean Acc: %4.2f" % (ti, acc, np.mean(np.array(acc_all)))
        )
    ...
    print(
        "Test Acc = %4.2f +- %4.2f%%" % (acc_mean, 1.96 * acc_std / np.sqrt(iter_num))
    )
...
if __name__ == "__main__":
    ...
    print(
        "Running time: %s Seconds: %s Min: %s Min per epoch"
        % (end - start, (end - start) / 60, (end - start) / 60 / iter_num)
    )
