...
from cross_domain_fsl.methods.gnn import GNN_nl
from cross_domain_fsl.methods import backbone_multiblock
from cross_domain_fsl.methods.tool_func import *
from cross_domain_fsl.methods.meta_template_StyleAdv_RN_GNN import MetaTemplate
...
class StyleAdvGNN(MetaTemplate):
    ...
    def __init__(self, model_func, n_way, n_support, tf_path=None):
        ...
        support_label = (
            torch.zeros(self.n_way * self.n_support, self.n_way)
            .scatter(1, support_label, 1)
    ...
    def forward_gnn(self, zs):
        ...
        scores = (
            scores.view(self.n_query, self.n_way, self.n_support + 1, self.n_way)[
                :, :, -1
            ]
            .permute(1, 0, 2)
            .contiguous()
    ...
    def adversarial_attack_Incre(self, x_ori, y_ori, epsilon_list):
        ...
        if "1" in blocklist and epsilon_list[0] != 0:
            ...
            x_normalized_block1 = (
                x_ori_block1 - ori_style_mean_block1.detach().expand(feat_size_block1)
            ) / ori_style_std_block1.detach().expand(feat_size_block1)
            x_ori_block1 = x_normalized_block1 * ori_style_std_block1.expand(
                feat_size_block1
            ) + ori_style_mean_block1.expand(feat_size_block1)
            ...
            ori_acc = (ori_pred == y_ori).type(torch.float).sum().item() / y_ori.size()[
                0
            ]
            ...
            adv_style_mean_block1 = fgsm_attack(
                ori_style_mean_block1, epsilon, grad_ori_style_mean_block1
            )
            adv_style_std_block1 = fgsm_attack(
                ori_style_std_block1, epsilon, grad_ori_style_std_block1
            )
        ...
        if "2" in blocklist and epsilon_list[1] != 0:
            ...
            x_ori_block1 = self.feature.forward_block1(x_ori)
            # update adv_block1
            x_adv_block1 = changeNewAdvStyle(
                x_ori_block1, adv_style_mean_block1, adv_style_std_block1, p_thred=0
            )
            # forward block2
            ...
            # contain ori_style_mean_block1 in the graph
            x_normalized_block2 = (
                x_ori_block2 - ori_style_mean_block2.detach().expand(feat_size_block2)
            ) / ori_style_std_block2.detach().expand(feat_size_block2)
            x_ori_block2 = x_normalized_block2 * ori_style_std_block2.expand(
                feat_size_block2
            ) + ori_style_mean_block2.expand(feat_size_block2)
            # pass the rest model
            x_ori_block3 = self.feature.forward_block3(x_ori_block2)
            x_ori_block4 = self.feature.forward_block4(x_ori_block3)
            x_ori_fea = self.feature.forward_rest(x_ori_block4)
            ...
            index = torch.randint(0, len(epsilon_list), (1,))[0]
            epsilon = epsilon_list[index]
            adv_style_mean_block2 = fgsm_attack(
                ori_style_mean_block2, epsilon, grad_ori_style_mean_block2
            )
            adv_style_std_block2 = fgsm_attack(
                ori_style_std_block2, epsilon, grad_ori_style_std_block2
            )
        ...
        if "3" in blocklist and epsilon_list[2] != 0:
            ...
            x_ori_block2 = self.feature.forward_block2(x_adv_block1)
            x_adv_block2 = changeNewAdvStyle(
                x_ori_block2, adv_style_mean_block2, adv_style_std_block2, p_thred=0
            )
            ...
            x_normalized_block3 = (
                x_ori_block3 - ori_style_mean_block3.detach().expand(feat_size_block3)
            ) / ori_style_std_block3.detach().expand(feat_size_block3)
            x_ori_block3 = x_normalized_block3 * ori_style_std_block3.expand(
                feat_size_block3
            ) + ori_style_mean_block3.expand(feat_size_block3)
            # pass the rest model
            x_ori_block4 = self.feature.forward_block4(x_ori_block3)
            x_ori_fea = self.feature.forward_rest(x_ori_block4)
            x_ori_output = self.classifier.forward(x_ori_fea)
            ...
            index = torch.randint(0, len(epsilon_list), (1,))[0]
            epsilon = epsilon_list[index]
            adv_style_mean_block3 = fgsm_attack(
                ori_style_mean_block3, epsilon, grad_ori_style_mean_block3
            )
            adv_style_std_block3 = fgsm_attack(
                ori_style_std_block3, epsilon, grad_ori_style_std_block3
            )
        ...
        return (
            ...
            adv_style_mean_block3,
            adv_style_std_block3,
        )
    ...
    def set_forward_loss_StyAdv(self, x_ori, global_y, epsilon_list):
        ...
        (
            ...
            adv_style_mean_block3,
            adv_style_std_block3,
        ) = self.adversarial_attack_Incre(x_ori, global_y, epsilon_list)
        ...
        acc_cls_ori = (scores_cls_ori.max(1, keepdim=True)[1] == global_y).type(
            torch.float
        ).sum().item() / global_y.size()[0]
        ...
        x_ori_z_stack = [
            torch.cat(
                [
                    x_ori_z[:, : self.n_support],
                    x_ori_z[:, self.n_support + i : self.n_support + i + 1],
                ],
                dim=1,
            ).view(1, -1, x_ori_z.size(2))
            for i in range(self.n_query)
        ]
        ...
        acc_cls_adv = (scores_cls_adv.max(1, keepdim=True)[1] == global_y).type(
            torch.float
        ).sum().item() / global_y.size()[0]
        ...
        x_adv_z_stack = [
            torch.cat(
                [
                    x_adv_z[:, : self.n_support],
                    x_adv_z[:, self.n_support + i : self.n_support + i + 1],
                ],
                dim=1,
            ).view(1, -1, x_adv_z.size(2))
            for i in range(self.n_query)
        ]
