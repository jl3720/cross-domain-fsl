...
from cross_domain_fsl.methods.tool_func import consistency_loss
        ...
        for i, (x_ori, global_y) in enumerate(train_loader_ori):
            ...
            if (total_it + 1) % 10 == 0 and self.tf_writer is not None:
                self.tf_writer.add_scalar(
                    "loss_fsl_ori:", loss_fsl_ori.item(), total_it + 1
                )
                self.tf_writer.add_scalar(
                    "loss_fsl_adv:", loss_fsl_adv.item(), total_it + 1
                )
                # self.tf_writer.add_scalar('loss_fsl_KL:', loss_fsl_KL.item(), total_it +1)
                self.tf_writer.add_scalar(
                    "loss_cls_ori:", loss_cls_ori.item(), total_it + 1
                )
                self.tf_writer.add_scalar(
                    "loss_cls_adv:", loss_cls_adv.item(), total_it + 1
                )
