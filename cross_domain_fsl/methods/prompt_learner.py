...
class PromptLearner(nn.Module):
    def __init__(self, classnames, clip_model):
        ...
        prompts_content = ['a {} style of a {}'.format(prompt_prefix, name) for name in classnames]
        prompts_style = [prompt_prefix]

        tokenized_prompts_content = torch.cat([clip.tokenize(p) for p in prompts_content]).to(device)
        tokenized_prompts_style = torch.cat([clip.tokenize(prompts_style)]).to(device)
        with torch.no_grad():
            embedding_contnet = clip_model.token_embedding(tokenized_prompts_content).type(dtype)
            embedding_style = clip_model.token_embedding(tokenized_prompts_style).type(dtype)

        # These token vectors will be saved when in save_model(),
        # but they should be ignored in load_model() as we want to use
        # those computed using the current class names
        self.register_buffer("token_prefix_content", embedding_contnet[:, :1, :])  # SOS
        self.register_buffer("token_suffix_content", embedding_contnet[:, 1 + n_ctx :, :])  # CLS, EOS
        self.register_buffer("token_prefix_style", embedding_style[:, :1, :])  # SOS
        self.register_buffer("token_suffix_style", embedding_style[:, 1 + n_ctx :, :])  # CLS, EOS
