...
from cross_domain_fsl.methods.backbone_multiblock import (
    Linear_fw,
    Conv2d_fw,
    BatchNorm2d_fw,
    BatchNorm1d_fw,
...
class Gconv(nn.Module):
    ...
    def __init__(self, nf_input, nf_output, J, bn_bool=True):
        ...
        if self.bn_bool:
            self.bn = (
                nn.BatchNorm1d(self.num_outputs, track_running_stats=False)
                if not self.maml
...
class Wcompute(nn.Module):
    ...
    def __init__(
        ...
        self.conv2d_1 = (
            nn.Conv2d(input_features, int(nf * ratio[0]), 1, stride=1)
            if not self.maml
            else Conv2d_fw(input_features, int(nf * ratio[0]), 1, stride=1)
        )
        self.bn_1 = (
            nn.BatchNorm2d(int(nf * ratio[0]), track_running_stats=False)
            if not self.maml
            else BatchNorm2d_fw(int(nf * ratio[0]), track_running_stats=False)
        )
        self.drop = drop
        if self.drop:
            self.dropout = nn.Dropout(0.3)
        self.conv2d_2 = (
            nn.Conv2d(int(nf * ratio[0]), int(nf * ratio[1]), 1, stride=1)
            if not self.maml
            else Conv2d_fw(int(nf * ratio[0]), int(nf * ratio[1]), 1, stride=1)
        )
        self.bn_2 = (
            nn.BatchNorm2d(int(nf * ratio[1]), track_running_stats=False)
            if not self.maml
            else BatchNorm2d_fw(int(nf * ratio[1]), track_running_stats=False)
        )
        self.conv2d_3 = (
            nn.Conv2d(int(nf * ratio[1]), nf * ratio[2], 1, stride=1)
            if not self.maml
            else Conv2d_fw(int(nf * ratio[1]), nf * ratio[2], 1, stride=1)
        )
        self.bn_3 = (
            nn.BatchNorm2d(nf * ratio[2], track_running_stats=False)
            if not self.maml
            else BatchNorm2d_fw(nf * ratio[2], track_running_stats=False)
        )
        self.conv2d_4 = (
            nn.Conv2d(nf * ratio[2], nf * ratio[3], 1, stride=1)
            if not self.maml
            else Conv2d_fw(nf * ratio[2], nf * ratio[3], 1, stride=1)
        )
        self.bn_4 = (
            nn.BatchNorm2d(nf * ratio[3], track_running_stats=False)
            if not self.maml
            else BatchNorm2d_fw(nf * ratio[3], track_running_stats=False)
        )
        self.conv2d_last = (
            nn.Conv2d(nf, num_operators, 1, stride=1)
            if not self.maml
            else Conv2d_fw(nf, num_operators, 1, stride=1)
        )
...
class GNN_nl(nn.Module):
    ...
    def forward(self, x):
        W_init = (
            torch.eye(x.size(1), device=x.device)
            .unsqueeze(0)
            .repeat(x.size(0), 1, 1)
            .unsqueeze(3)
        )
